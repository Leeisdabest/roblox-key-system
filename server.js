  ...
  if (url.pathname === "/" || url.pathname === "/generate-key") {
    ...
    if (pending) {
      ...
      return page(
        ...
        "Finishing Key",
        `<h1>Almost Ready</h1>
        <p>Linkvertise sent you back. Your key will unlock in a few seconds.</p>
        <div class="timer" id="waitTimer">${waitSeconds}</div>
        <button id="claimButton" disabled>Wait ${waitSeconds} seconds...</button>
        <script>
          ...
          const interval = setInterval(() => {
            secondsLeft -= 1;
            waitTimer.textContent = secondsLeft;

            if (secondsLeft <= 0) {
              clearInterval(interval);
              claimButton.disabled = false;
              claimButton.textContent = "Generate Key";
    ...
    return page(
      ...
      <a href="/go" id="openLink">Click Here To Open Linkvertise</a>
      <div id="fallback" class="hidden">
        <p>If Linkvertise does not bring you back automatically, use your browser back button and press this after you finish.</p>
