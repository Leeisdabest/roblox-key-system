...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === "/checkpoint") {
    ...
    if (pass !== UNLOCK_PASS || !pending || pending.step !== 1) {
      saveKeys(keys);
      return page(
        res,
        "Checkpoint Locked",
  ...
  if (url.pathname === "/final-checkpoint" || url.pathname === "/return" || url.pathname === "/unlock") {
    ...
    if (!pending || pending.step !== 2) {
      const fallbackSession = realSession || makeSession();
      ...
      keys.__pending[fallbackSession] = fallbackPending;
      saveFlow(keys, req, fallbackSession, 2, fallbackPending.createdAt);
      saveKeys(keys);
      return page(
        res,
        "Checkpoint 1 Complete",
        `<section class="hero">
          ...
          <h1>One more step</h1>
          <p>Your first checkpoint is accepted. Open checkpoint 2 and let it redirect you back here to generate your 24 hour key.</p>
          <div class="actions">
            <a class="primary" href="/go2">Open Checkpoint 2</a>
          </div>
        </section>`,
        {
          "Set-Cookie": sessionCookieSet(fallbackSession),
        }
      );
    }
